...
usage() {
  cat <<'HELP'
Usage:
  sudo bash scripts/install_amazon_linux.sh [options]
  curl -fsSL https://raw.githubusercontent.com/OWNER/REPO/main/scripts/install_amazon_linux.sh | sudo bash -s -- --github-repo OWNER/REPO
...
if [ "${EUID:-$(id -u)}" -ne 0 ]; then
  echo "Please run as root, for example: sudo bash scripts/install_amazon_linux.sh" >&2
  exit 1
fi
...
if [ "$ASSUME_YES" -eq 0 ]; then
  echo "$APP_NAME Amazon Linux installer"
